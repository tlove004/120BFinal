...
// the list is ordinated by the ASCII codes 
...
void Set(volatile unsigned char *port, unsigned char  dq, unsigned char state) {
	if (state)
		*port |= (1<<dq);
	else
		*port &= ~(1<<dq);
...
void lcd_init() {
	//Set(controlPort, RST, 1); //LCD_PORT |= LCD_RST_PIN;
	...
	//Set(controlPort, RST, 0);
	//delay_ms(64);
	//Set(controlPort, RST, 1);

    // Disable LCD controller
	Set(controlPort, SCE, 0);
...
void lcd_send(unsigned char data, LcdCmdData cd)
{
	...
	Set(controlPort, DC, (cd == LCD_DATA));
	
	for(unsigned char i=0;i<8;i++) {
		// Set the DATA pin value:8 bits in a row
		Set(controlPort, DATA, ((data>>(7-i)) & 0x01));
		
		// Toggle the clock
		Set(controlPort, CLK, 1);
		Set(controlPort, CLK, 0);
	}

	// Disable display controller
	Set(controlPort, SCE, 1);
	
	// Data/DC can be used as button inputs when not sending to LCD (/w pullups)
	//LCD_DDR &= ~(LCD_DATA_PIN | LCD_DC_PIN);
	Set(dataPort, DATA, 0);
	Set(dataPort, DC, 0);
	Set(controlPort, DATA, 1);
	Set(controlPort, DC, 1);
}
